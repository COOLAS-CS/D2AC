...
class D2AC:
        ...
        if self.delay_mode == "obs":
            ...
            for i in range(1,delay_max+1,1):
                ...
                target_q, target_q_bound = self.__compute_target_q(
                    rew[i], done[i], q.detach(), q_next_sample.detach(), log_prob_act2.detach(), i
                )
    ...
    def __compute_target_q(self, r, done, q, q_next, log_prob_a_next, index):
        gamma_power = self.gamma ** index
        target_q = r + (1 - done) * gamma_power * (
